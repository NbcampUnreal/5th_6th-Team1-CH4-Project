...
class TPSWARSURVIVE_API UWeaponComponent : public UActorComponent
{
	GENERATED_BODY()

private://DataAsset을 받아온다
	UPROPERTY(EditAnywhere, Category = "DataAsset")
	class UDataAsset* DataAssets[(int32)EWeaponType::Max];

public://무기 Type에 맞는지 확인해주는 함수들
...
private:
	bool IsIdleMode();	//StateComponent, WeaponComponent 둘 다 같은 레벨이다. 서로 소유할 수 없는 관계이기 때문에
						//참조만 해서 리턴받기 위해 IsIdleMode를 사용한다.

public:
	class AAttachment* GetAttachment();
...
public: //무기가 바뀌었을때 통보해줄 delgate
	FWeaponTypeChanged OnWeponTypeChange;
